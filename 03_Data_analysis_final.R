...
# =========================================================================
# Core library loading with version checking
required_packages <- c(
...
check_available_fonts <- function() {
  ...
  # Check recommended fonts for scientific publications
  publication_fonts <- available_fonts %>% 
    filter(str_detect(family, "Arial|Helvetica|Times|Calibri")) %>% 
    dplyr::select(family, style) %>% 
    distinct()
  
  return(publication_fonts)
}

# Get best available font for publications
get_best_publication_font <- function() {
  available_fonts <- systemfonts::system_fonts()$family
  
  # Font priority for scientific publications
  ...
  return("")  # Empty string lets R use default font
}

# Check available fonts
cat("Available publication-style fonts on your system:\n")
print(check_available_fonts())

# Get best font
best_font <- get_best_publication_font()

# =========================================================================
# Setup directory for saving results
# Define the base directory
base_dir <- "Results_V7.1"

# Define the subdirectories
sub_dirs <- c("figures", "tables")
...
# =========================================================================
# -------------------------------------------------------------------------
# Professional Academic Visual System (Scientific Publication Standards)
# -------------------------------------------------------------------------

# Professional color palette system for scientific publications
publication_palettes <- list(
  # Primary findings color scheme (based on scientific publication guidelines)
  biophysical_zones = c(
    "IDZ" = "#0173B2",      # Blue: low risk
    "BTZ" = "#DE8F05", # Orange: transition period
    "CTZ" = "#CC78BC",      # Purple: high risk zone (highlight)
    "SDZ" = "#029E73",      # Green: post-peak
    "Very High NDVI" = "#56B4E9",      # Light blue: very high vegetation
    "Unknown" = "#999999"              # Gray: unknown
  ),
  
  # Professional color scheme for vegetation indices
  indices = c(
    "NDVI" = "#1B9E77", "EVI" = "#D95F02", 
    "LAI" = "#7570B3", "FPAR" = "#E7298A"
  ),
  
  # Scientific color scheme for climate zones
  climate = c(
    "Temperate" = "#2166AC",
  ...
  # Ecological color scheme for vegetation types
  vegetation = c(
...
# Professional academic theme (strictly following publication guidelines)
# Optimized theme based on your original theme
publication_theme_professional <- theme_minimal(base_size = 10, base_family = best_font) +
  theme(
    # Font settings - ensure all text elements use correct font
    ...
    # Axis settings - compliant with publication standards
    ...
    axis.line = element_line(color = "#000000", linewidth = 0.3),  # Standard axis lines
    ...
    # Label settings (important for annotate)
...
# To avoid font warnings, create a safe annotate function
...
cat("\n=== Font Configuration ===\n")
cat("Selected font for publication theme:", best_font, "\n")
...
saveFigure <- function(plot, filename, width = 8, height = 6, dpi = 600) {
  ...
  if(!is.null(plot$data)) {
    data_filename <- file.path(base_dir, "tables", paste0(filename, "_data.csv"))
    tryCatch({
...
# Data preprocessing and quality control - enhanced multi-source vegetation data processing
preprocess_data <- function(data) {
  ...
  # Geographic classification (based on latest climate classification standards Köppen-Geiger climate classification)
  ...
  # ========== Enhanced multi-source vegetation data processing ==========
  ...
  if("MODIS_IGBP" %in% colnames(data)) {
    ...
    # Create simplified classification system (for subsequent comparison)
  ...
  if("Hansen_Tree_Cover_2000_Percent" %in% colnames(data)) {
    ...
    # Retain detailed forest cover grading (keep original code)
  ...
  # 2. Create multi-source consistency indicators
  if(all(c("MODIS_IGBP_Simplified", "Hansen_Simplified") %in% colnames(data))) {
    # MODIS and Hansen consistency
  ...
  if(all(c("MODIS_IGBP_Simplified", "Copernicus_LC_Simplified") %in% colnames(data))) {
    # MODIS and Copernicus consistency
  ...
  if(all(c("Copernicus_LC_Simplified", "Hansen_Simplified") %in% colnames(data))) {
    # Copernicus and Hansen consistency
  ...
  # 3. Create three-source comprehensive consistency score
  if(all(c("MODIS_IGBP_Simplified", "Copernicus_LC_Simplified", "Hansen_Simplified") %in% colnames(data))) {
    ...
    data$Multi_Source_Consistency_Score <- apply(
      ...
      function(x) {
        ...
        return(4 - unique_values)  # 3: full agreement, 2: two agree, 1: all disagree
      }
    )
  ...
  # 4. Create unified vegetation type labels (priority: MODIS > Copernicus > Hansen)
  ...
  # Vegetation index change rate calculation
...
# Filter out data where "Climate_Zone" is not "Polar", Copernicus_LC_Class is not "Permanent water bodies"
# and MODIS_IGBP_Class is not "Water Bodies"
...
# a standard requirement for scientific publications.
# -------------------------------------------------------------------------
...
analyze_vegetation_thresholds <- function(data) {
  ...
  create_supplementary_density_plot <- function(plot_data, ndvi_breakpoints) {
    ...
    p <- ggplot(plot_data, aes(x = NDVI_1)) +
      ...
      publication_theme_professional
    saveFigure(p, "Supplementary_Figure_1_NDVI_Kernel_Density", width = 7, height = 5)
    cat("  ✓ Supplementary Figure saved.\n")
  }
  
  # --- Helper function 2: Supplementary cross-index consistency table (optimized version) ---
  analyze_cross_index_consistency <- function(data_with_zones, all_results) {
    ...
    # Using `janitor` package's `tabyl` and `adorn_totals` is the most concise, warning-free method
    # Make sure janitor is installed: if(!require(janitor)) install.packages("janitor")
    ...
    # Convert janitor table to standard data frame and save
  ...
  # --- 1. Execute core breakpoint analysis ---
  ...
  # --- 2. Define risk zones and add to data ---
  ...
  if(all(c("NDVI_1", "EVI_1") %in% names(analysis_results))) {
    # Pass data that already includes NDVI_Biophysical_Zone column
...
create_figure_1_plots <- function(analysis_results, processed_data) {
  ...
  plot_list <- list()
  publication_palettes <- get("publication_palettes", envir = .GlobalEnv)
  ...
  create_breakpoint_plot <- function(idx, result) {
    ...
    # Define plot boundaries - increase boundary distance to maintain distance between points/lines and plot frame
    plot_xlim <- c(x_range[1] - 0.08*x_span, x_range[2] + 0.08*x_span)
    
    # For NDVI/EVI, use slightly expanded range
    ...
    p <- ggplot(df, aes(x = x, y = y)) +
      ...
      geom_line(data = pred_data_extended, aes(x = x, y = y), 
                color = publication_palettes$indices[gsub("_1", "", idx)], 
                linewidth = 1.2) +
    ...
    if(nrow(conf_int) >= 1) {
      for(i in 1:min(2, nrow(conf_int))) {
        ...
        p <- p + geom_polygon(data = conf_data, 
                              aes(x = x, y = y),  # Use column names directly, don't inherit
                              fill = "#E31A1C", 
                              alpha = 0.15,
                              inherit.aes = FALSE)  # Explicitly don't inherit aesthetic mappings
      }
    }
    ...
    p <- p +
      publication_theme_professional +
      # Keep clip = "on" but increase boundary space
      coord_cartesian(xlim = plot_xlim,
                      ylim = c(-0.02 * y_max, y_max * 1.1), # Add space both below and above
  ...
  create_distribution_plot <- function(data_with_biophysical_zones, ndvi_result) {
    ...
    p <- ggplot(full_dist, aes(x = NDVI_Biophysical_Zone, y = Percentage, fill = NDVI_Biophysical_Zone)) +
      ...
      scale_fill_manual(values = c("Unknown" = "grey70", publication_palettes$biophysical_zones), guide = "none") +
  ...
  print(final_figure)
  saveFigure(final_figure, "Main_Figure_1_Data-driven_discovery_of_CTZ", width = 8.3, height = 6.8)
...
# -------------------------------------------------------------------------
# MAIN FIGURE 2: Data analysis section - Multi-source vegetation data validation and cross-platform consistency
# -------------------------------------------------------------------------
...
analyze_vegetation_validation_data <- function() {
  ...
  data <- landslide_data_processed # Use previously processed data
  analysis_results <- list()
  ...
  # Manually extract needed values from cohen_kappa_result and create a new data frame
  kappa_df <- data.frame(
  ...
  # Helper function for analyzing 2C, 2D, 2E
  ...
  analyze_zone_distribution <- function(data_subset, x_var_str, output_filename) {
    summary_df <- data_subset %>%
      filter(!is.na(.data[[x_var_str]]), !is.na(NDVI_Biophysical_Zone),
             ...
             NDVI_Biophysical_Zone != "Unknown") %>%
      # First grouping: group by x-axis category and risk zone together, calculate count for each combination
      group_by(across(all_of(x_var_str)), NDVI_Biophysical_Zone) %>%
      plyr::summarise(Count = n(), .groups = "drop") %>%
      # Second grouping (key step): group only by x-axis category
...
analyze_vegetation_validation_data <- function() {
  ...
  # Use global variables generated in preprocessing steps
  ...
  # --- Helper function for analyzing 2C, 2D, 2E (this is the key correction part) ---
  analyze_zone_distribution <- function(data_subset, x_var_str, output_filename) {
    summary_df <- data_subset %>%
      dplyr::filter(!is.na(.data[[x_var_str]]), !is.na(NDVI_Biophysical_Zone),
                    .data[[x_var_str]] != "Unknown", .data[[x_var_str]] != "Other", 
                    NDVI_Biophysical_Zone != "Unknown") %>%
      # First grouping: group by x-axis category and risk zone together
      ...
      # Calculate percentage, where sum(Count) is the total within each x-axis category
      dplyr::mutate(Percentage = Count / sum(Count) * 100) %>%
      # Ungroup after calculation completion, this is good practice
      dplyr::ungroup()
  ...
  # Assign analysis results to global variable for use by subsequent visualization functions
...
# -------------------------------------------------------------------------
# MAIN FIGURE 2: Visualization section - Multi-source vegetation data validation and cross-platform consistency (improved x-axis label display)
# -------------------------------------------------------------------------
create_vegetation_validation_plots <- function(analysis_results, data) {
  ...
  # --- 2A: Global vegetation type distribution (Treemap) --- [No changes]
  ...
  # --- 2B: Multi-source consistency levels (pie chart) --- [No changes]
  ...
  # --- Modified shared function: Create risk distribution plots (for 2C, 2D, 2E) ---
  # Main change: Added label processing logic to display long labels in two lines
  create_zone_distribution_plot <- function(summary_df, x_var_str, x_lab, is_first_plot = FALSE, is_last_plot = FALSE) {
    ...
    # Create new modified data frame, process labels for two-line display
    modified_df <- summary_df
    
    # Special handling for forest cover labels - more concise format
    if(x_var_str == "Forest_Cover_Category") {
      # Use more concise labels, keeping only essential information
      label_mapping <- c(
      ...
      if("order" %in% colnames(modified_df)) {
        ...
        # If no order column, try to create one based on standard sorting
    ...
    # Handle other types of labels [same as before]
    else if(x_var_str == "MODIS_IGBP_Simplified") {
      # Land cover type label handling [code unchanged]
      label_mapping <- c(
      ...
      # Climate zone label handling [code unchanged]
      label_mapping <- c(
        "Temperate" = "Temperate",
    ...
    # Use more compact width setting for forest cover charts
    bar_width <- 0.8  # Default width
    if(x_var_str == "Forest_Cover_Category") {
      bar_width <- 0.7  # Forest cover uses narrower bars
    }
    
    # Create chart using modified labels
    p <- ggplot(modified_df, aes(x = Display_Label, y = Percentage, fill = NDVI_Biophysical_Zone)) +
      ...
      scale_fill_manual(values = publication_palettes$biophysical_zones, name="Biophysical Zone") +
      scale_y_continuous(labels = scales::percent_format(scale=1), 
                         limits = c(0, 100), breaks = seq(0,100,25)) +
      publication_theme_professional
    
    # Use special label settings for forest cover charts
    if(x_var_str == "Forest_Cover_Category") {
      p <- p + theme(
      ...
      # Other charts use standard settings
      p <- p + theme(
    ...
    # Only add Y-axis labels to first plot
    if (is_first_plot) {
  ...
  p2e <- create_zone_distribution_plot(
    ...
    x_lab = "Climate Zone",
    is_first_plot = FALSE,
    is_last_plot = FALSE  # This is the last plot, show legend
  )
  ...
  # Combine Figure 2 - use similar size ratios as Figure 1
  ...
  bottom_row <- p2c + p2d + p2e + 
    ...
    theme(legend.position = "bottom")  # Ensure legend is at bottom
  ...
  # Save combined plot - use same size as Figure 1
  saveFigure(main_fig2, "Main_Figure_2_Multi-platform_validation", width = 8.3, height = 6.8)  # Same size as Figure 1
...
# 1.1 Create a unified, simplified final classification (as the right endpoint of the Sankey diagram)
...
# ==============================================================================
# Step 2: Define colors and labels (same as your original code)
# ==============================================================================
...
# ==============================================================================
# Step 3: Prepare final plotting data (same as your original code)
# ==============================================================================
...
plot_data <- dplyr::bind_rows(
  ...
  # Ensure Source is also an ordered factor for plotting
  dplyr::mutate(
...
# ==============================================================================
# Step 4: Add percentage labels - optimize label positions (final corrected version with explicit package names)
# ==============================================================================

# First, we need to calculate the total size of each classification block (stratum) from the original plotting data
...
# Second, based on the total size of each block, we calculate its position on the Y-axis and percentage
label_data <- stratum_summary %>%
  # Use desc() to reverse the arrangement order to match ggalluvial's stacking order (put first factor level at top)
  dplyr::arrange(Source, dplyr::desc(stratum_ordered)) %>%
  # Group by x-axis variable (Source)
  dplyr::group_by(Source) %>%
  # Calculate top, bottom, and center positions for each color block
  dplyr::mutate(
    y_top = cumsum(stratum_total_n),
    y_bottom = dplyr::lag(y_top, default = 0),
    y_center = (y_top + y_bottom) / 2,
    # Calculate correct percentage based on block total size
  ...
  dplyr::ungroup() # Ungroup after calculation completion

# Finally, add styling (size and color) for labels
...
# ==============================================================================
# Step 5: Draw Sankey diagram (using new label_data)
# ==============================================================================
...
sankey_plot <- ggplot2::ggplot(data = plot_data,
  ...
  # Use ggalluvial core geometric objects
  ...
  # Add percentage labels within classification blocks - using corrected label_data
  ggplot2::geom_text(
    data = label_data %>% dplyr::filter(label != ""), # Only label non-empty labels
  ...
  ggplot2::scale_fill_manual(
    ...
    breaks = class_order,
    na.value = "grey80" # Specify color for possible NA values
  ) +
  ...
  ggplot2::scale_x_discrete(
    ...
    expand = ggplot2::expansion(mult = c(0.05, 0.05)) # Adjust white space on both sides of x-axis
  ) +
  
  # Y-axis doesn't need extra space
  ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = c(0.01, 0.01))) +
  
  # Theme and title
  # Note: If you want black background, apply theme_dark() here or set manually
  # For example: theme_dark() + theme(...)
  publication_theme_professional +
  ...
  ggplot2::theme(
    ...
    legend.spacing.x = ggplot2::unit(0.2, "cm"), # Reduce internal legend spacing
    legend.key.size = ggplot2::unit(0.5, "cm"), # Reduce legend symbol size
...
# Execute plotting
saveFigure(sankey_plot, "Supplementary_Figure_2_sankey_plot", width = 8.3, height = 6.8)  # Same size as Figure 1
...
# ------------------------------------------------------------------------------
# Data analysis section - Prepare data for Figure 3
# ------------------------------------------------------------------------------
prepare_data_for_figure_3 <- function() {
    ...
    if(nrow(zone_data) > 200) { 
      tryCatch({
        ...
        p <- ggplot(result$data, aes(x = x, y = y)) +
          ...
          geom_line(data = pred_data, aes(x=x, y=y), color = publication_palettes$climate[zone], linewidth=1.2) +
  ...
  if(length(all_climate_plots) >= 1) {
    ...
    saveFigure(supplementary_fig_y, "Supplementary_Figure_3_Faceted_Breakpoints", width=12, height=4)
    cat("✓ Supplementary Figure 3 saved to 'figures/' directory.\n")
  } else {
    cat("Warning: Could not generate faceted plots for any climate zones.\n")
  }
  
  # Initialize variables in the return list
  seasonal_summary <- NULL
  
  # Prepare data for Figure 3C (seasonality)
  if(!"Season" %in% colnames(data)){
    ...
    if(nrow(chi_sq_data) > 0) {
      chi_sq_tests <- chi_sq_data %>%
        ...
        tidyr::nest() %>%
        dplyr::mutate(
          # Use purrr::map to safely apply tests
          chi_sq_result = purrr::map(data, ~ tryCatch(chisq.test(.x$Count), error = function(e) NULL)),
          # Only extract results when test is successful
  ...
  # Initialize variables in the return list to prevent subsequent steps from failing
  temporal_summary <- NULL
  temporal_change_annotations <- NULL
  
  # Prepare data for Figure 3D (time series)
  temporal_cols <- paste0("NDVI_", 5:1)  # Modified to order from 5 to 1
  if(!all(temporal_cols %in% colnames(data))){
    ...
    temporal_data_long <- data %>%
      ...
      # Add reverse order index for sorting
    ...
    if(nrow(temporal_data_long) > 0){
      ...
      temporal_change_annotations <- temporal_summary %>%
        ...
        # Note: now we go from large to small, so we select less than rather than greater than 1
      ...
      print(mk_results)
      saveTable(mk_results, "Supplementary_Table_2_Mann_Kendall_Results")
...
# ------------------------------------------------------------------------------
# Visualization section - Draw Figure 3 (final refined version)
# ------------------------------------------------------------------------------
...
create_figure_3_visualization <- function(figure_data, width = 8.3, height = 6.8) {
  ...
  # Figure 3A: Vegetation index distribution - legend at bottom
  ...
  # [!] This is the corrected complete code block for generating Figure 3B
  ...
  if (!is.null(figure_3_data$data_3b) && nrow(figure_3_data$data_3b) > 0) {
    ...
    saveTable(dunn_summary, "Supplementary_Table_1_Pairwise_Dunn_Test")
  ...
  # Figure 3C: Seasonal distribution - legend at bottom, x-axis labels in two lines
  if(!is.null(figure_data$data_3c) && nrow(figure_data$data_3c) > 0) {
    # Modify NDVI category labels for two-line display
    ...
    # Calculate total sample size for each NDVI category
    ...
    plot_3c <- ggplot(figure_data$data_3c, aes(x = NDVI_Category_2line, y = Percentage, fill = Season)) +
      ...
      # Add sample size information below bars
      annotate("text", 
  ...
  # Figure 3D: Time series analysis - legend at bottom
  if(!is.null(figure_3_data$data_3d_summary) && nrow(figure_3_data$data_3d_summary) > 0) {
    # [!] Core modification part: Create detailed X-axis labels
    # --------------------------------------------------------------------------
    # 1. Define time window day ranges
    time_window_days <- c(
      "T5" = "(80-64 days)",
      "T4" = "(64-48 days)",
      "T3" = "(48-32 days)",
    ...
    # 2. Create a label vector with line breaks, order consistent with X-axis (T5 -> T1)
    ...
    # e.g., biophysical_zone_colors <- publication_palettes$biophysical_zones
    ...
    plot_3d <- ggplot(figure_3_data$data_3d_summary, 
      ...
      geom_point(size = 2.5) +
      # Modified here: adjust label position, move to upper left
      geom_text(data = figure_data$data_3d_annotations,
                aes(label = sprintf("%+.1f%%", Percent_Change)),
                vjust = 1.8, hjust = 0.6, # Add hjust=0.8 to move labels left
                nudge_x = -0.15, # Move labels left
      ...
      # [!] Use our newly created labels in scale_x_discrete
      # --------------------------------------------------------------------------
    scale_x_discrete(
      name = "Time Window (days before event)", # Update X-axis title
      labels = detailed_x_labels,            # Use new labels
      expand = expansion(mult = c(0.1, 0.15)) # Adjust left-right margins to accommodate labels
    ) +
      # --------------------------------------------------------------------------
    # --- Modified here: increase right margin ---
    coord_cartesian(ylim = c(0.5, NA), clip = "off") + # Keep clip="off" so text can expand when needed
      #scale_x_discrete(expand = expansion(mult = c(0.05, 0.05))) + # Increase more white space on right side
      ...
      theme(
        ...
        legend.box.margin = ggplot2::margin(t = -5),
        # Add this line to ensure labels don't get clipped
  ...
  # Also save individual subplots for separate use
  saveFigure(final_figure, "Main_Figure_3_Mechanistic_understanding", width = 8.3, height = 6.8)
...
# Task: Prepare data, execute cross-validation, train final model, and save all results
# =========================================================================
run_fig4_modeling <- function() {
  ...
  roc_plot <- ggplot(all_classification_df, aes(d = Response, m = Prediction_Prob, color = as.factor(Fold))) +
    ...
    theme_minimal() +
    # Modification point 1: Move legend to inside upper right corner of plot frame
  ...
  # --- 5. Prepare and save all data needed for plotting ---
  ...
  fig4_results_data <- list(
    all_predictions = all_regression_preds,       # For Panel A and B
    avg_importance = avg_importance,         # For Panel C
    pdp_summary = pdp_summary,               # For Panel D
    analysis_results_global = fig1_analysis_results # Pass Figure 1 results
  )
  
  # Create directory and save
  dir.create("models", showWarnings = FALSE)
  saveRDS(fig4_results_data, "models/fig4_plotting_data.rds")
  # Also save final model and features for Figure 5 use
...
# Task: Load saved model results, generate four panels of Figure 4 and final combined figure
# =========================================================================
create_fig4_plots <- function() {
  cat("\n=== CREATING PLOTS FOR FIGURE 4 ===\n")
  
  # --- 1. Load data needed for plotting ---
  ...
  # Unpack data from list into independent variables for convenience
  ...
  # --- 2. Generate four visualization panels (code almost completely identical to your previous version) ---
  ...
  # Modification point 2: Simplify x-axis labels for small plot c
  ...
  plot_4c <- ggplot(head(avg_importance, 10), aes(x = reorder(SimpleFeature, Mean_Importance), y = Mean_Importance, fill = Category)) +
    ...
    # Modification point 2: Remove legend title
    ...
    theme(
      ...
      # Add the following lines to set legend background and border
      legend.background = element_rect(
        colour = "grey80",   # Border color, can be set to "grey50" or any color you want
        linewidth = 0.3,     # Border width, can be adjusted
        fill = "white"      # Legend background fill color, default is "white", you can change as needed
      )
    ) 
  ...
  # --- 3. Assemble and save final Figure 4 ---
  ...
  saveFigure(main_fig4, "Main_Figure_4_Non-circular_predictive_modeling_independently_confirm", width = 8.3, height = 6.8)
...
# This function will read data from files, runs very fast
