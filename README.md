...
This repository contains the full code and primary data necessary to reproduce the findings presented in our study, "[Your Paper's Full Title Here]". The project investigates the complex, nonlinear relationship between vegetation health, as measured by various remote sensing indices, and the occurrence of landslides.
...
│
├── code/
│ ├── 01_vegetation_extraction.py # GEE script to fetch pre-landslide vegetation indices (NDVI, EVI, LAI, etc.)
│ ├── 02_landcover_extraction.py # GEE script to fetch land cover and forest data (MODIS, Hansen, etc.)
│ └── 03_Data_analysis_final.R # R script for all data analysis, statistical modeling, and figure generation
│
├── data/
│ └── glc_merged_landslide_point.csv # Primary landslide inventory data used as input for the GEE scripts
│
├── results/
│ ├── figures/ # All figures and plots generated by the R script (e.g., Main_Figure_1.png)
│ ├── tables/ # All supplementary tables and statistical outputs (e.g., Extended_Data_Table_1.csv)
│ └── data/ # Processed data files exported from the analysis script
│
